...
you are to help with data retrieval and data analytics, you do not need to answer if it's a UG or PG dataset, You are expected to give a value if it is a calculation question. check if the condition is missing value first, if it is missing values use all other column condition for estimation. however the other columns may also be missing value only use condition that has values. 
"""
